#!/usr/bin/env bash
# Train vggt_fusion_distill (--fusion_distill_alpha 0.2), then eval_benchmark_rewrite.
set -euo pipefail
...
echo "[$(date -Iseconds)] START train_alignment_experiments (vggt_fusion_distill)" | tee -a "${LOG_TRAIN}"

python train_alignment_experiments.py \
  --model_mode vggt_fusion_distill \
  --data_dir "${DATA_DIR}" \
