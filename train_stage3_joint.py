...
# -*- coding: utf-8 -*-
"""
Stage3：encoder（LoRA）+ adapter + grasp head 一起训练。
加载 Stage2 的 ckpt，解冻 head（及 adapter、LoRA），联合优化。
"""
...
def main():
    ...
    parser.add_argument("--lr_head", type=float, default=1e-3, help="head 与 adapter 用较大 lr")
    parser.add_argument("--weight_decay", type=float, default=1e-2)
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--save_dir", type=str, default=None)
    parser.add_argument("--save_name", type=str, default="gc6d_lift3d_stage3.pt")
    ...
    parser.add_argument("--use_images", action="store_true", help="与 Stage1/2 一致：点云+图像双模态")
    ...
    if not args.ckpt_stage2:
        args.ckpt_stage2 = os.path.join(ROOT, "checkpoints", "gc6d_lift3d_stage2.pt")
    ...
    use_images = args.use_images or (ckpt2_meta.get("encoder_type") == "lift3d_clip_multimodal")
    ...
    sd = ckpt2_meta.get("model", ckpt2_meta)
    model.load_state_dict(sd, strict=True)

    # 解冻 LoRA（仅最后 n 个 block）、adapter、head（多模态时含 fusion）
    ...
    head_params = list(model.grasp_head.parameters()) + list(model.encoder.adapter.parameters())
    if use_images:
        head_params += list(model.fusion.parameters())

    lora_params = model.encoder.get_backbone_lora_params(last_n_blocks=args.lora_last_n_blocks or None)
    ...
    for step in range(args.max_steps):
        ...
        if (step + 1) % 100 == 0 or step == 0:
            logger.info("[Stage3] step %d/%d loss=%.6f", step + 1, args.max_steps, loss.item())
        if val_loader and (step + 1) % args.val_every == 0:
            ...
            if val_n > 0:
                ...
                logger.info("[Stage3] step %d val_loss=%.6f (n=%d)", step + 1, val_loss, val_n)
