#!/bin/bash
# 小样本 + GraspNet head：VGGT 微调三阶段。S2 步数最多，S3=300+early stop，与 run_small_vggt_ft.sh 一致。
set -e
cd "$(dirname "$0")"
...
USE_MULTI_GT="${USE_MULTI_GT:-0}"
LOG_DIR="${LOG_DIR:-logs/vggt_ft_small_graspnet_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$LOG_DIR"
echo "VGGT ft small GraspNet | N=$N S1=$STEPS1_VGGT S2=$STEPS2_VGGT S3=$STEPS3_VGGT best_gt_w=$LOSS_BEST_GT_WEIGHT pred2gt_frac=$PRED2GT_TOP_FRAC match=$MATCH_MODE wd_s2=$WEIGHT_DECAY_S2 wd_s3=$WEIGHT_DECAY_S3 | log=$LOG_DIR"
exec > >(tee -a "$LOG_DIR/run.log") 2>&1
echo "========== VGGT ft Stage1 (graspnet head, best_gt=$LOSS_BEST_GT_WEIGHT pred2gt_frac=$PRED2GT_TOP_FRAC match=$MATCH_MODE) =========="
python train_vggt_ft_stage1.py \
  ...
  --save_name gc6d_vggt_small_ft_graspnet_stage1.pt
echo "========== VGGT ft Stage2 =========="
python train_vggt_ft_stage2.py \
  ...
  --ckpt_stage1 checkpoints/gc6d_vggt_small_ft_graspnet_stage1.pt \
  --log_dir "$LOG_DIR" \
  --save_name gc6d_vggt_small_ft_graspnet_stage2.pt
echo "========== VGGT ft Stage3 (lora_last_n_blocks=$LORA_LAST_N_BLOCKS, early_stop_val_worse=${EARLY_STOP_VAL_WORSE:-0}) =========="
EXTRA_STAGE3=""
[ "$USE_MULTI_GT" = "1" ] && EXTRA_STAGE3="--use_multi_gt"
python train_vggt_ft_stage3.py \
  ...
  --head_dropout $HEAD_DROPOUT --adapter_dropout $ADAPTER_DROPOUT --loss_17d --loss_best_gt_weight $LOSS_BEST_GT_WEIGHT --pred2gt_top_frac $PRED2GT_TOP_FRAC --match_mode $MATCH_MODE \
  --ckpt_stage2 checkpoints/gc6d_vggt_small_ft_graspnet_stage2.pt \
  --log_dir "$LOG_DIR" \
  --save_name gc6d_vggt_small_ft_graspnet_stage3.pt \
  --save_best_ckpt $EXTRA_STAGE3
echo "Done. Checkpoints: checkpoints/gc6d_vggt_small_ft_graspnet_stage*.pt | Logs: $LOG_DIR"
