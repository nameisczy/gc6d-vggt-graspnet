#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""VGGT base：backbone 注入 LoRA，训 LoRA + adapter + head（与 LIFT3D/VGGT ft 配置对齐）。"""
...
from models import build_vggt_base_policy
...
def main():
    ...
    p.add_argument("--save_name", default="gc6d_vggt_base.pt")
    ...
    p.add_argument("--lora_r", type=int, default=8, help="LoRA 秩，与 LIFT3D/VGGT ft 对齐")
    p.add_argument("--lora_scale", type=float, default=1.0, help="LoRA scale")
    ...
    model = build_vggt_base_policy(
        encoder_feat_dim=256, lora_r=args.lora_r, lora_scale=args.lora_scale,
    ...
    trainable = list(model.encoder.get_backbone_lora_params()) + list(model.encoder.adapter.parameters()) + list(model.grasp_head.parameters())
    ...
    for step in range(args.max_steps):
        ...
        if (step + 1) % 100 == 0 or step == 0:
            logger.info("[VGGT base] step %d/%d loss=%.6f", step + 1, args.max_steps, loss.item())
        if val_loader and (step + 1) % args.val_every == 0:
            ...
            model.train()
            if val_n > 0:
                logger.info("[VGGT base] step %d val_loss=%.6f (n=%d)", step + 1, val_loss_sum / val_n, val_n)
        if loss.item() < 1e-6:
            break
    ...
    torch.save({"model": model.state_dict(), "encoder_type": "vggt_base", "grasp_head_type": getattr(model, "grasp_head_type", "simple"), "grasp_head_num_proposals": getattr(model, "grasp_head_num_proposals", None), "lora_r": args.lora_r, "lora_scale": args.lora_scale, "step": step + 1, "loss": loss.item()}, path)
