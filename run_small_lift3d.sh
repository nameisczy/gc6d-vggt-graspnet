#!/bin/bash
# 小样本：仅 LIFT3D 三阶段。S2 单独微调 encoder 步数最多；S3=300+early stop 防 joint 爆。
set -e
cd "$(dirname "$0")"
...
LOG_DIR="${LOG_DIR:-logs/lift3d_small_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$LOG_DIR"
echo "LIFT3D small | N=$N S1=$STEPS1_LIFT S2=$STEPS2_LIFT S3=$STEPS3_LIFT (S1+S3=$STEPS_BASE) wd_s2=$WEIGHT_DECAY_S2 wd_s3=$WEIGHT_DECAY_S3 | log=$LOG_DIR"
exec > >(tee -a "$LOG_DIR/run.log") 2>&1
# LIFT3D = 点云 encoder 仅，与 VGGT（图像）对比
echo "========== LIFT3D Stage1 (点云 only) =========="
python train_stage1_freeze_encoder.py --data_dir "$DATA" --max_samples $N --batch_size $BS --max_steps $STEPS1_LIFT --lr $LR1_LIFT --weight_decay $WEIGHT_DECAY --lift3d_root "$LIFT3D_ROOT" --log_dir "$LOG_DIR" --lora_r $LORA_R --lora_scale $LORA_SCALE --val_every $VAL_EVERY --head_dropout $HEAD_DROPOUT --adapter_dropout $ADAPTER_DROPOUT --save_name gc6d_lift3d_small_stage1.pt
echo "========== LIFT3D Stage2 =========="
python train_stage2_lora_encoder.py --data_dir "$DATA" --max_samples $N --batch_size $BS --max_steps $STEPS2_LIFT --lr $LR2_LIFT --weight_decay $WEIGHT_DECAY_S2 --lora_r $LORA_R --lora_scale $LORA_SCALE --val_every $VAL_EVERY --head_dropout $HEAD_DROPOUT --adapter_dropout $ADAPTER_DROPOUT --ckpt_stage1 checkpoints/gc6d_lift3d_small_stage1.pt --lift3d_root "$LIFT3D_ROOT" --log_dir "$LOG_DIR" --save_name gc6d_lift3d_small_stage2.pt
...
python train_stage3_joint.py --data_dir "$DATA" --max_samples $N --batch_size $BS --max_steps $STEPS3_LIFT --lr $LR3_LIFT --lr_head $LR_HEAD3_LIFT \
  ...
  --ckpt_stage2 checkpoints/gc6d_lift3d_small_stage2.pt --lift3d_root "$LIFT3D_ROOT" --log_dir "$LOG_DIR" --save_name gc6d_lift3d_small_stage3.pt
echo "Done. Checkpoints: checkpoints/gc6d_lift3d_small_stage*.pt | Logs: $LOG_DIR"
