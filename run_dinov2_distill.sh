#!/usr/bin/env bash
# Train lift3d_replacement_distill_dinov2, then eval_benchmark_rewrite on the saved checkpoint.
set -euo pipefail
...
echo "[$(date -Iseconds)] START train_alignment_experiments (lift3d_replacement_distill_dinov2)" | tee -a "${LOG_TRAIN}"

python train_alignment_experiments.py \
  --model_mode lift3d_replacement_distill_dinov2 \
  --data_dir "${DATA_DIR}" \
