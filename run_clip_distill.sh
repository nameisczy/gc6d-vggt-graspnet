#!/usr/bin/env bash
# Train lift3d_replacement_distill_clip, then eval_benchmark_rewrite on the saved checkpoint.
set -euo pipefail
...
# GC6D / graspclutter6dAPI camera id (use realsense-d435 for "realsense" eval views)
...
echo "[$(date -Iseconds)] START train_alignment_experiments (lift3d_replacement_distill_clip)" | tee -a "${LOG_TRAIN}"

python train_alignment_experiments.py \
  --model_mode lift3d_replacement_distill_clip \
  --data_dir "${DATA_DIR}" \
