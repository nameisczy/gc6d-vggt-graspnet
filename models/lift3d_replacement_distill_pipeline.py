# -*- coding: utf-8 -*-
"""
LIFT3D-CLIP / DINOv2 feature distillation：teacher = GraspNet backbone seed；
student = encoder → projector → LN → adapter；仅训 adapter 路径，encoder 冻结。
"""
...
class Lift3DClipReplacementDistillGraspNet(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encoder_type = "lift3d_replacement_distill_clip"
        self.model_mode = "lift3d_replacement_distill_clip"
...
class Dinov2ReplacementDistillGraspNet(nn.Module):
    def __init__(
        self,
        ...
        *,
        ...
        self.grasp_net = grasp_net
        self.distill_loss_type = str(distill_loss_type)
        self.distill_projector = make_conv1d_projector(768, 256)
        self.distill_ln = nn.LayerNorm(256, elementwise_affine=False)
        self.distill_adapter = _make_distill_adapter()
        self.encoder_type = "lift3d_replacement_distill_dinov2"
        self.model_mode = "lift3d_replacement_distill_dinov2"
        self.requires_images = False

    def forward(self, point_cloud: torch.Tensor, images: Optional[torch.Tensor] = None) -> dict:
        if images is not None:
            raise ValueError("Dinov2ReplacementDistillGraspNet 仅点云")
        end_points = {"point_clouds": point_cloud}
        view_estimator = self.grasp_net.view_estimator
        backbone = view_estimator.backbone
        f_teacher, seed_xyz, end_points = backbone(point_cloud, end_points)
...
def build_dinov2_replacement_distill_graspnet(
    *,
    graspnet_ckpt: str,
    graspnet_root: Optional[str] = None,
    distill_loss_type: str = "l2",
    device: Optional[torch.device] = None,
) -> Dinov2ReplacementDistillGraspNet:
    import torch as _t
    ...
    grasp_net = load_graspnet_pretrained(graspnet_ckpt, device, graspnet_root, is_training=False)
    model = Dinov2ReplacementDistillGraspNet(
        dinov2_encoder=enc,
        grasp_net=grasp_net,
        distill_loss_type=distill_loss_type,
    )
