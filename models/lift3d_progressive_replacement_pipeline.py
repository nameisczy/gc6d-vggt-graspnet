# -*- coding: utf-8 -*-
"""
LIFT3D-CLIP / DINOv2 渐进式替代：f_in = (1-alpha)*f_graspnet + alpha*f_lift3d_adapted；
encoder 冻结；仅训 projector + LayerNorm + progressive_adapter（及可选 head，默认冻结）。
"""
...
class Lift3DClipProgressiveReplacementGraspNet(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encoder_type = "lift3d_progressive_replacement_clip"
        self.model_mode = "lift3d_progressive_replacement_clip"
...
class Dinov2ProgressiveReplacementGraspNet(nn.Module):
    def __init__(
        self,
        dinov2_encoder: DINOv2SeedEncoder,
        grasp_net: nn.Module,
        *,
        progressive_alpha: float = 0.5,
    ):
        ...
        self.progressive_adapter = _make_progressive_adapter()
        self.use_adapter = False
        self.encoder_type = "lift3d_progressive_replacement_dinov2"
        self.model_mode = "lift3d_progressive_replacement_dinov2"
        self.requires_images = False

    def forward(self, point_cloud: torch.Tensor, images: Optional[torch.Tensor] = None) -> dict:
        if images is not None:
            raise ValueError("Dinov2ProgressiveReplacementGraspNet 仅点云")
        end_points = {"point_clouds": point_cloud}
        view_estimator = self.grasp_net.view_estimator
        backbone = view_estimator.backbone
        f_graspnet, seed_xyz, end_points = backbone(point_cloud, end_points)
        enc_b768s = self.encoder(point_cloud, seed_xyz)
        x = self.replacement_projector(enc_b768s.float())
        x_ln = self.progressive_ln(x.permute(0, 2, 1).contiguous()).permute(0, 2, 1).contiguous()
        f_lift = self.progressive_adapter(x_ln)
        a = self.progressive_alpha
        seed_features = (1.0 - a) * f_graspnet + a * f_lift
...
def build_lift3d_clip_progressive_replacement_graspnet(
    *,
    ...
    clip_backbone = _load_lift3d_clip_backbone_pretrained(root)
...
def build_dinov2_progressive_replacement_graspnet(
    *,
    graspnet_ckpt: str,
    graspnet_root: Optional[str] = None,
    progressive_alpha: float = 0.5,
    device: Optional[torch.device] = None,
) -> Dinov2ProgressiveReplacementGraspNet:
    import torch as _t
    ...
    grasp_net = load_graspnet_pretrained(graspnet_ckpt, device, graspnet_root, is_training=False)
    model = Dinov2ProgressiveReplacementGraspNet(
        dinov2_encoder=enc,
        grasp_net=grasp_net,
        progressive_alpha=progressive_alpha,
    )
