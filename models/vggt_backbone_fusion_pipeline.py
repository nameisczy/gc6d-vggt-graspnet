# -*- coding: utf-8 -*-
"""
VGGT（冻结）与 GraspNet backbone seed 的显式融合路径（Part 2）：
...
def _make_fusion_adapter() -> nn.Sequential:
...
class VGGTFusionProgressiveGraspNet(nn.Module):
    """渐进式将归一化后的 VGGT seed 特征与 GraspNet backbone seed 混合。"""
    ...
    def __init__(
        ...
        *,
        ...
        self.fusion_to_seed_projector = make_conv1d_projector(vggt_dim, 256)
        self.fusion_seed_ln = nn.LayerNorm(256, elementwise_affine=False)
        self.fusion_seed_adapter = _make_fusion_adapter()
        self.encoder_type = "vggt_progressive_fusion"
        self.model_mode = "vggt_progressive_fusion"
        self.requires_images = True

    def forward(self, point_cloud: torch.Tensor, images: Optional[torch.Tensor] = None) -> dict:
        if images is None:
            raise ValueError("VGGTFusionProgressiveGraspNet 需要 images")
        ...
        x = self.fusion_to_seed_projector(vggt_raw)
        x_ln = self.fusion_seed_ln(x.permute(0, 2, 1).contiguous()).permute(0, 2, 1).contiguous()
        f_vggt_fused = self.fusion_seed_adapter(x_ln)

        a = self.progressive_alpha
        seed_features = (1.0 - a) * f_graspnet + a * f_vggt_fused
...
class VGGTFusionDistillGraspNet(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.encoder_type = "vggt_fusion_distill"
        self.model_mode = "vggt_fusion_distill"
        self.requires_images = True

    def forward(self, point_cloud: torch.Tensor, images: Optional[torch.Tensor] = None) -> dict:
        if images is None:
            raise ValueError("VGGTFusionDistillGraspNet 需要 images")
        end_points = {"point_clouds": point_cloud}
        view_estimator = self.grasp_net.view_estimator
        backbone = view_estimator.backbone
        ...
        pts_n = normalize_xyz_with_pc(point_cloud, pts)
        vggt_raw = nearest_neighbor_gather_features(seed_n, pts_n, feat_b768k).float()
        x = self.fusion_distill_projector(vggt_raw)
        x_ln = self.fusion_distill_ln(x.permute(0, 2, 1).contiguous()).permute(0, 2, 1).contiguous()
        f_student = self.fusion_distill_adapter(x_ln)
...
def build_vggt_fusion_progressive_graspnet(
    *,
    graspnet_ckpt: str,
    ...
    model = VGGTFusionProgressiveGraspNet(
        encoder=encoder,
        grasp_net=grasp_net,
...
def build_vggt_fusion_distill_graspnet(
    *,
    graspnet_ckpt: str,
    graspnet_root: Optional[str] = None,
    vggt_ckpt: Optional[str] = None,
    ...
    grasp_net = load_graspnet_pretrained(graspnet_ckpt, device, graspnet_root, is_training=False)
    model = VGGTFusionDistillGraspNet(
        encoder=encoder,
        grasp_net=grasp_net,
