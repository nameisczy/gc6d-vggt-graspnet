#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""VGGT 微调 Stage2：冻结 head+adapter，只训 encoder。"""
...
def main():
    ...
    p.add_argument("--save_name", default="gc6d_vggt_ft_stage2.pt")
    p.add_argument("--ckpt_stage1", default=None)
    ...
    ckpt1 = args.ckpt_stage1 or os.path.join(ROOT, "checkpoints", "gc6d_vggt_ft_stage1.pt")
    if not os.path.isfile(ckpt1):
        raise FileNotFoundError(ckpt1)
    ckpt1_meta = torch.load(ckpt1, map_location="cpu", weights_only=False)
    ...
    model = build_vggt_ft_policy(
        encoder_feat_dim=256, ckpt_path=None, freeze_backbone=False,
    ...
    model.load_state_dict(ckpt1_meta.get("model", ckpt1_meta), strict=True)
    for p in model.grasp_head.parameters():
        p.requires_grad = False
    model.encoder.set_adapter_trainable(False)
    model.encoder.set_backbone_lora_trainable(True)

    lora_params = [p for p in model.encoder.get_backbone_lora_params() if p.requires_grad]
    optim = torch.optim.AdamW(lora_params, lr=args.lr, weight_decay=args.weight_decay)
    val_loader = None
    if args.val_every > 0:
        try:
    ...
    for step in range(args.max_steps):
        ...
        if (step + 1) % 100 == 0 or step == 0:
            logger.info("[VGGT ft Stage2] step %d/%d loss=%.6f", step + 1, args.max_steps, loss.item())
        if val_loader and (step + 1) % args.val_every == 0:
            ...
            model.train()
            if val_n > 0:
                logger.info("[VGGT ft Stage2] step %d val_loss=%.6f (n=%d)", step + 1, val_loss_sum / val_n, val_n)
        if loss.item() < 1e-6:
            break
    ...
    torch.save({"model": model.state_dict(), "encoder_type": "vggt_ft", "grasp_head_type": getattr(model, "grasp_head_type", "simple"), "grasp_head_num_proposals": getattr(model, "grasp_head_num_proposals", None), "lora_r": args.lora_r, "lora_scale": args.lora_scale, "stage": 2, "step": step + 1, "loss": loss.item()}, path)
