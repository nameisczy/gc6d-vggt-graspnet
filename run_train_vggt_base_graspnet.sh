#!/bin/bash
# VGGT base + GraspNet head：全量数据，单阶段。与 run_train_vggt_base.sh 同结构，仅 head 为 graspnet。
set -e
cd "$(dirname "$0")"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}"

DATA="${DATA:-/mnt/ssd/ziyaochen/GraspClutter6D/offline_unified}"
LOG_DIR="${LOG_DIR:-logs/vggt_base_graspnet_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$LOG_DIR"
...
echo "VGGT base GraspNet full-data | steps=$STEPS lr=$LR | best_gt_w=$LOSS_BEST_GT_WEIGHT pred2gt_frac=$PRED2GT_TOP_FRAC match=$MATCH_MODE | log=$LOG_DIR | val_every=$VAL_EVERY"
...
python train_vggt_base.py \
  ...
  --val_every $VAL_EVERY \
  --save_name gc6d_vggt_base_graspnet.pt

echo "Done. Checkpoint: checkpoints/gc6d_vggt_base_graspnet.pt | Logs: $LOG_DIR"
